...
double tm_i2c_getcore0(unsigned char slot) {
	...
	do {
		usleep(10000);
		v = tm_i2c_Data2Core(tm_i2c_req(tm_i2c_fd, (TM_ADDR >> 1) + (slot >> 1), TM_GET_CORE0, 0));
		t++;
...
double tm_i2c_getcore1(unsigned char slot) {
	...
	do {
		usleep(10000);
		v = tm_i2c_Data2Core(tm_i2c_req(tm_i2c_fd, (TM_ADDR >> 1) + (slot >> 1), TM_GET_CORE1, 0));
		t++;
	} while(t <= TM_TRIES && (v == 0 || v > 3.3));
	if (v == 0 || v > 3.3) v = -1;
	return v;
...
double tm_i2c_gettemp(unsigned char slot) {
	if (slot < 0 || slot > 31) return 0;
	return tm_i2c_Data2Temp(tm_i2c_req(tm_i2c_fd, (TM_ADDR >> 1) + (slot >> 1), TM_GET_TEMP, 0));
}
/*
...
unsigned int tm_i2c_set_vid(unsigned char slot, unsigned char vid) {
	...
	do {
		res = tm_i2c_req(tm_i2c_fd, (TM_ADDR >> 1) + (slot >> 1), (slot & 1) ? TM_SET_CORE1 : TM_SET_CORE0, vid);
		tr++;
